...
def add_note(args, notes: NoteBook) -> str:
    """
    Generate a new note and add it to the notebook 
    if a note with the same title doesn't already exist.
    """
    title = args[0]
    text = " ".join(args[1:])
...
def remove_note(args, notes: NoteBook) -> str:
    """Remove a note from the provided NoteBook based on the given title."""
    title = args[0]
    if notes.find_by_title(title):
...
def find_note(args, notes: NoteBook) -> str:
    """
    A function to find a note in a notebook by its title 
    and return the text of the note.
    """
    title = args[0]
...
def show_all_notes(notes: NoteBook) -> str:
    """A function to display all notes in a notebook."""
    all_notes_str = ""
